...
Module("Cactus.Data", function (m) {
  ...
  var Collection = {
    ...
    isCollection : function (collection) {
      ...
      // Fix for when NodeList is a function (Safari 5).
      if ("NodeList" in glob && collection instanceof NodeList) {
        return true;
      }
      return !!(collection &&
     ...
     * @return boolean
     */
    none : function (collection, pred) {
      return !Collection.any(collection, pred);
    },
    /**
     ...
     * @return boolean
     */
    notAll : function (collection, pred) {
      return !Collection.all(collection, pred);
    },
    /**
     ...
     * @return Array
     */
    replicate : function (count, element) {
      var a = [];
    ...
    /**
     * Pairs up each element in as with an element in bs and returns a
     * list of these pairs.
     *
     * @param collection as
     * @param collection bs
    ...
    zip : function (as, bs) {
      var res = [];
      for (var i = 0, ii = Math.min(as.length, bs.length); i < ii; i++) {
        res.push([as[i], bs[i]]);
      }
      return res;
    }
