 ...
 * @return <V>
 */
Function.prototype.id = function (v) {
  return v;
};
/**
 * Given a function f, returns a new function g that when called calls f but
 * does not pass any arguments along to f.
 * f is called in the global scope.
 *
 * Useful when methods are used as a callback and you want to assure that no
 * args are passed along.
 *
 * It's possible to bind/curry f before freezing it.
 *
 * @return Function
 *   A function that calls f with no arguments.
 */
Function.prototype.freeze = function () {
  var f = this;
  return function () {
    f();
  };
