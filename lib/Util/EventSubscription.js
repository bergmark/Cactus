...
Module("Cactus.Util", function (m) {
  ...
  var EventSubscription = Joose.Role("EventSubscription", {
    has : {
      /**
    ...
    methods : {
      ...
      subscribe : function (eventName, subscriber, removeOnTrigger) {
        ...
        if (!(subscriber instanceof Function)) {
          var methodName = "on" + eventName + "Triggered";
          if (!(subscriber[methodName] instanceof Function)) {
            throw new Error(
              "EventSubscription:subscribe: Subscribing object does not implement " + methodName);
          }
        }
