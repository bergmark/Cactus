...
Joose.Module("Cactus.Util", function (m) {
    ...
    methods : {
      ...
      getErrorString : function () {
        ...
        for (var p in this.errors) {
          errorStrings.push(this._getErrorString(p, this.errors[p]));
        }
  ...
  var Options = Joose.Class("Options", {
    has : {
      definition : null,
      errorMessage : null
    },
    methods : {
      ...
      parse : function (options, throwErrors) {
        ...
        if (this.definition.validator) {
          // Validators.
          if (this.definition.validator(options)) {
            return options;
          } else {
            return fail("", "Validation failed, got " + options + ". " + validationMessage);
          }
        } else if (this.definition.enumerable) {
          // Enumerables.
          if (collection.hasValue(this.definition.enumerable, options)) {
            return options;
          }
          return fail("", "Expected a value in [" + this.definition.enumerable + "], but got " + options);
          ...
          if ("defaultValue" in this.definition) {
            ...
            return this.definition.defaultValueFunc();
          } else {
            return options;
          }
