...
Module("Cactus.Util", function (m) {
  ...
  var T_Union = Class({
    ...
    methods : {
      ...
      check : function (actualType, fail) {
        if (!collection.hasValue(this.types, actualType)) {
          fail("Expected a Union");
        }
      }
    }
  });
  ...
  var Options = Class("Options", {
    ...
    methods : {
      BUILD : function (definition, throwErrors) {
        ...
        if (!(definition instanceof Array) && !("type" in definition) && !("enumerable" in definition) && !(definition.type instanceof Type)) {
      ...
      parse : function (options, throwErrors) {
        ...
        } else if (this.definition.type instanceof Type) {
          var actualType = this._typeof(options);
          this.definition.type.check(actualType, fail.curry(""));
