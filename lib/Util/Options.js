    ...
    methods : {
      ...
      check : function (actualType, value, em) {
        if (!(value instanceof Object)) {
          em.add("", 'Expected %s, but got "%s"'.format(JSON.stringify(this.getName()), actualType));
          return value;
        }
        ...
        for (var p in this.type) if (this.type.hasOwnProperty(p)) {
          if (!(p in value) && (this.type[p].required || !("required" in this.type[p]))
            ...
            var opts = new Options(this.type[p], false);
    ...
    methods : {
      BUILD : function (definition) {
        ...
        if (definition.union) {
          ...
          this.definition.type = new T_Hash(definition.type);
        }
      },
