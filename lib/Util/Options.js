...
Module("Cactus.Util", function (m) {
  ...
  var Union = Class({
    ...
    methods : {
      ...
      check : function (actualType) {
        return collection.hasValue(this.types, actualType);
      }
    }
  });
  ...
  var Options = Class("Options", {
    ...
    methods : {
      BUILD : function (definition, throwErrors) {
        ...
        if (definition.union) {
          definition.type = new Union(definition.union);
          delete definition.union;
        }

        // No need for type if it's an Array type.
        if (!(definition instanceof Array) && !("type" in definition) && !("enumerable" in definition) && !("union" in definition)) {
      ...
      parse : function (options, throwErrors) {
        ...
        } else if (this.definition.type instanceof Union) {
          if (!this.definition.type.check) {
            return;
          }
          var actualType = this._typeof(options);
          if (!this.definition.type.check(actualType)) {
            fail("", "Expected a type in a Union");
          }
