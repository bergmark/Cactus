  ...
  var T_Map = Class({
    ...
    methods : {
      ...
      check : function (actualType, value, em) {
        ...
        var m = {};
        for (var p in value) {
          var option = new Options({ type : this.innerType });
      ...
      check : function (actualType, value, em) {
        if (!(value instanceof this._constructor)) {
          em.add("", 'Expected an instance of "%s", but got value <%s> (type "%s")'
                 .format(this._getName(), value, actualType));
        }
        return value;
      },
      _getName : function () {
    ...
    methods : {
      BUILD : function (definition) {
        ...
        if (definition.union) {
          ...
          this.definition.type = definition.type;
        }
      },
      parse : function (options, throwErrors) {
        ...
        if (this.definition.type instanceof Type && options !== null && options !== undefined) {
          var actualType = Options.typeof(options);
          options = this.definition.type.check(actualType, options, this.errorMessage);
          if (this.errorMessage.has() && throwErrors) {
            throw new Error(this.errorMessage.getErrorString());
          }
        } else if (this.definition.type instanceof Object && options instanceof Object
          ...
          for (var p in this.definition.type) if (this.definition.type.hasOwnProperty(p)) {
            if (!(p in options) && (this.definition.type[p].required || !("required" in this.definition.type[p]))
                && !("defaultValue" in this.definition.type[p])
                && !("defaultValueFunc" in this.definition.type[p])) {
              this.errorMessage.add(p, "Missing property");
            } else {
              var opts = new Options(this.definition.type[p], false);
              h[p] = opts.parse(options[p], false);
              if (opts.hasErrors()) {
                this.errorMessage.addSubObj(p, opts._getErrorMessage());
              }
            }
          }
          ...
          throwError();
          options = h;
        } else if (this.definition.type === typeof options && options !== null) {
          // Standard types: string, number, boolean.
        } else if (this.definition.required === false && (options === null || options === undefined)) {
