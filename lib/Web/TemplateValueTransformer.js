...
Module("Cactus.Web", function (m) {
  ...
  Class("TemplateValueTransformer", {
    does : ViewHandler,
    has : {
      /**
       ...
       */
      keyPathTransformers : { init : O.new },
      /**
       * @type Array<Hash>
       *   Stores information regarding added value transformers for
       *   specific selectors.
       */
      selectorTransformers : { init : A.new }
    }, methods : {
      BUILD : function (view) {
        return {
          view : view
      ...
      clone : function (rootElement) {
        var clone = new m.TemplateValueTransformer(rootElement);
        ...
        if (option.selector) {
          ...
          if (selectorTransformers.length > 0) {
            ...
            if (option.selector === "root") {
              ...
              elements = $(option.selector, this.getView());
            }
