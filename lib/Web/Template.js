...
Module("Cactus.Web", function (m) {
  ...
  function Template(rootElement) {
    ...
    refresh : function () {
      ...
      this.classNameConditions.attach();
    },
    /**
    ...
    bindTo : function (dataSource) {
      ...
      this.classNameConditions.bindTo(dataSource);
    ...
    _setValue : function (keyPath, value) {
      ...
      var transformedValue = this.valueTransformers.transformForKeyPath(keyPath, value);

      for (var i = 0, elements = this.elements.get(keyPath); i < elements.length; i++) {
        var element = elements[i];
        var selectorTransformedValue = this.valueTransformers.transformForElement(element, transformedValue);
    ...
    _findKeyPathForElement : function (element) {
      ...
      for (var i = 0, keyPaths = this.elements.get(); i < keyPaths.length; i++) {
        var keyPath = keyPaths[i];
        for (var j = 0, elements = this.elements.get(keyPath); j < elements.length; j++) {
    ...
    onValueChangedTriggered : function (object, keyPath) {
      ...
      for (var i = 0, keys = this.elements.keys(); i < keys.length; i++) {
        ...
        // > String.hasPrefix
        if (keyPath === keys[i].substring(0, keyPath.length)) {
          this.update(keys[i]);
        }
      }
    },
     ...
     */
    _clone : function () {
      var rootElement = this.getRootElement().cloneNode(true);
